...
public class CataloguesManager : MonoBehaviour
{
    ...
    public void CatalogueSelectionChangedEvent()
    {
        ...
        if(useTmpCatalogue)
        {
            ...
            Global.tmpCatalogue ??= new(-1, "Neuer Fragenkatalog" + (newCatalogueNameCounter > 0 ? (" " + newCatalogueNameCounter.ToString()) : ""), -1, 0, 0, 0, new(), new()); // If null -> make a new one.
    ...
    public void QuestionSelectionChangedEvent()
    {
        ...
        if (selectedQuestionName == "Neu" && questionAddRecursionFailsafe == false)
        {
            ...
            Question newQuestion = new(-1, "Frage" + (newQuestionCounter + 1).ToString(), "", 0, 0, -1, answers, new List<AnswerHistory>());
